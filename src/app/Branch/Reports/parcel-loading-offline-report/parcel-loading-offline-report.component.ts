...
export class ParcelLoadingOfflineReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localData) {
      try {
        ...
        this.data2 = Array.isArray(stateData?.data) ? stateData.data : [];
        this.fromDate = stateData?.fromDate || '';
        this.toDate = stateData?.toDate || '';
      } catch (e) {
  ...
  ExportEXcel(): void {
    ...
    if (this.pfdata) {
      headerData.push([this.pfdata.companyName]);
      headerData.push([`Address: ${this.pfdata.location} - ${this.pfdata.branchId?.name} | Phone No: ${this.pfdata.phone}`]);
    }

    headerData.push(['Parcel Loading Offline Report']);
    headerData.push([`From: ${this.formatDate(this.fromDate)} To: ${this.formatDate(this.toDate)}`]);
    headerData.push([`Print Date: ${this.formatDate(this.today)} Time: ${this.formatTime(this.today)}`]);
    headerData.push([]);
